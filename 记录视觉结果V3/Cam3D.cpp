...
//========================================
bool cCam3D::Init(char* IP_A,u_short port_A,char* IP_B,u_short port_B)
{
	CamA.IP = IP_A;
	CamA.port = port_A;
	CamB.IP = IP_B;
	...
	CamC.IP = "192.168.64.98";
	CamC.port = 2002;

	CamD.IP = "192.168.64.99";
	CamD.port = 2002;
...
/*	if (dataFile.m_bStartSave)
	{
		...
		{
			dataFile.SaveTraceData( CamA.x_cur  , CamA.y_cur , CamA.t_cur  , 1 ,1,1,1,88);
		}
		if(CamB.dataReadFlag)
		{
			dataFile.SaveTraceData( CamB.x_cur  , CamB.y_cur , CamB.t_cur  , 2 ,1,1,1,88);
		}
		if(CamC.dataReadFlag)
		{
			dataFile.SaveTraceData( CamC.x_cur  , CamC.y_cur , CamC.t_cur  , 3 ,1,1,1,88);
		}
		if(CamD.dataReadFlag)
		{
			dataFile.SaveTraceData( CamD.x_cur  , CamD.y_cur , CamD.t_cur  , 4 ,1,1,1,88);
		}
	}*/
	...
	if(CamC.FilterState && CamD.FilterState )return 2;
	if(CamA.FilterState && CamB.FilterState )return 1;
